...
UPdating: 2019-10-05，庆祝建国70周年，地图修改为中国红！
...
解压缩后，产量数据在TS_FI_AQUACULTURE.csv文件中。凡纳滨对虾的产量数据，可以通过其3位种类代码是"PNV"进行查找。当前可以获得的最新即2017年的产量数据。
...
world_PNV_production_2017_dt <-
  world_PNV_production_dt[YEAR == 2017]
```
...
world_PNV_production_2017_country_info_dt <-
  merge(
    world_PNV_production_2017_dt,
...
world_PNV_production_2017_country_info_dt[1:5]
# 累加每个国家的总产量，凡纳滨对虾可在不同环境下养殖，如海、淡水、半咸水等
PNV_production_2017_per_country_dt <-
  world_PNV_production_2017_country_info_dt[, .(Production = sum(QUANTITY)), keyby =
                                              c("Name_En")]
#按照产量降序排列
setorder(PNV_production_2017_per_country_dt,-Production)
setnames(PNV_production_2017_per_country_dt,
         c("Name_En"),
         c("Country"))
PNV_production_2017_per_country_dt
```
...
FAO_nomatched_country_name_v <-
  PNV_production_2017_per_country_dt$Country[!(PNV_production_2017_per_country_dt$Country %in% unique(map_world_dt$region))]
FAO_nomatched_country_name_v
...
PNV_production_2017_per_country_dt[Country %chin% FAO_nomatched_country_name_v, 
                                   Country := maps_matched_country_name_v]
#把凡纳滨对虾产量数据跟地图数据合并
map_world_PNV_production_2017_dt <-
  merge(
    map_world_dt,
    PNV_production_2017_per_country_dt,
    all.x = TRUE,
...
map_world_PNV_production_2017_dt[Production == 0, Production:=NA]
#计算国家经纬度均值，用于定位国家名称的显示位置
map_world_PNV_country_position_2017_dt <-
  map_world_PNV_production_2017_dt[Production > 0, 
                                   .(MeanLong = mean(long),
                                     MeanLat = mean(lat),
                                     group = mean(group)),
                                   by = c("region")]
map_world_PNV_country_position_2017_dt[1:5]
```
...
国家名，根据map_world_PNV_country_position_2017_dt数据集中的平均经纬度定位显示，并且加了一个蓝色圆点。
...
showtext_auto()
ggplot(map_world_PNV_production_2017_dt,
  ...
  geom_point(aes(x =MeanLong, y= MeanLat),
             color = "green", 
             size = 1.5,
             data=map_world_PNV_country_position_2017_dt) +
  geom_text_repel(
  ...
  annotate(
    geom = 'text',
      label = '数据来源：FAO, 2019.  http://www.fao.org/fishery/statistics/global-aquaculture-production/zh',
    x = 10, y = -55,
    size = 4,
    family = 'heiti',
    color = 'grey50',
