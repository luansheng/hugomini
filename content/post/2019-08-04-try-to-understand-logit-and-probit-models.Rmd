...
Logit和probit连接函数，是在处理二分变量或者分类变量时，经常用到的两个函数。但对于为什么要使用它们的原因并不是很清楚。本文试着把近期看到的一些材料理解并记录下来。
...
利用ISLR包中的信用卡还款违约数据集Default来作为例子说明，为什么线性模型不适合处理分类数据。数据集的探索性分析见散点和箱线图。
...
针对信用开还款是否违约，建立一个线性模型：
$$y_{i}=\alpha+\beta x_{i}+\varepsilon_{i}$$

$y_{i}$是一个二分变量，表示第i个样本的信用卡还款状态，违约=1，不违约=0；**如果分析的是信用开不违约的发生率，那么把违约=0，不违约=1**；$\varepsilon_{i}$为独立残差，$\varepsilon_{i}$~$N(0,\sigma^{2}_{\varepsilon})$ 。如果x是随机的，应该与$\varepsilon$的关系是独立。        
$y_{i}$的期望可以表述为：
...
$$
上式中，$j^{*}$表示基线变量，$x_{ik}$表示第i个体第k个自变量，$\beta_{kj}$表示第j个类别第k个自变量（解释变量）的回归系数。
...
四个截距intercept实际上给出的是在参考水平下（Lake=Hancock，size=small<2.3，Gender=male）的log-odd值。譬如，在参考水平下，即Hancock湖小型雄性短尾鳄胃中食物Bird vs Fish的log-odd值为-1.94358，odd值为$e^{-1.94358}$=`r exp(-1.94358)`。也表明在该湖中鱼类居多，鸟类较少。
...
发生率Odd为$e^{-1.03811}$=`r exp(-1.03811)`。这表明，这尾短尾鳄胃中食物是Invertebrated的概率远低于是鱼的概率。
...
=1.01763
$$
发生率Odd为$e^{1.01763}$=`r exp(1.01763)`。这表明，这尾短尾鳄胃中食物是Invertebrate的概率要高于是鱼的概率。

#### 2.4.2 Adjacent-category Logit
在基线logit模型中，分类是无序的。现在考虑分类是有序的情况下，该如何建立更有优化的模型。譬如对虾卵巢的发育等级，眼柄的磨损程度等性状，均为有序性状。对于无序分类，我们选择其中一个类别作为基线类。而对于有序分类，这样处理更有意义：
...
2,3,4,...,r分别作为基线类。邻接分类的比较用于致死率分析会更有意义，

r-1个邻接Logit方程（连接函数）定义如下：
...
= -L_{r-1}-L_{r-2}+...-L_{1}
$$
上述公示表明，基于邻接logit模型可以转换基线logit模型。

#### 2.4.3 The proportional-Odds Cumulative Logit Model （百分比发生率累积Logit模型）
这个模型可能是对于有序分类最为常用。ASReml中好像用的也是这个模型。         
假设分类变量为$y=1,2,3,...,r$，每个类别发生的概率分别为$\mu_{1}, \mu_{2},...,\mu_{r}$，并且$P(y_{i} \le r)=\mu_{1} + \mu_{2} + ... + \mu_{r}$。
 
假定存在一个潜在变量（latent variable）$y^{*}$，代表事件发生的可能性，并且与解释变量$x$之间存在一种线性关系:
$$
y_{i}^{*} = \alpha+\beta_{1} x_{i1}+...+\beta_{k} x_{ik}+\varepsilon_{i}
$$
如果是二分变量，$y_{i}^{*}$划分为2个区域；如果是多分类，那么会被划分为r个区域，有r-1个边界。设定r-1个边界值为：$\alpha_{1}<\alpha_{2}<\alpha_{3}...<\alpha_{r-1}$，分类变量的值为：
...
$$
其中$y_{i}$表示第i个样本落在1-j类别之间的概率，$j=1,2,3...,r-1$。假定残差项$\varepsilon$独立分布，符合logistic或者标准正态分布，上式实际上为累积分布函数。

如果残差项$\varepsilon$符合logistic分布，那么有序的logit模型为：
$$
logit(P(y_{i} \le j)) = ln(\frac{P(y_{i} \le j)}{P(y_{i} > j)}) \\
=  \alpha_{j}-(\alpha+\beta_{1} x_{i1}+...+\beta_{k} x_{ik}) \\
= \alpha_{j} - \alpha -\beta_{1} x_{i1}-...-\beta_{k} x_{ik}
$$
反之：
$$
logit(P(y_{i} > j)) = ln(\frac{P(y_{i} > j)}{P(y_{i} \le j)}) \\
= -(\alpha_{j} - \alpha -\beta_{1} x_{i1}-...-\beta_{k} x_{ik}) \\
= \alpha-\alpha_{j}+\beta_{1} x_{i1}+...+\beta_{k} x_{ik}
$$


上述公式跟二分变量很相似。假设有四个有序分类$r=1, 2, 3, 4$，那么logit模型为：
$$
...
$$
注意$L_{1}, L_{2},...,L_{r-1}$是累积发生率，可以看作是**类似于事件发生的阈值**。

上述Logit方程的几个特点：

* 所有Logi回归方程的斜率都是一样的，即不同logit方程具有相同的一套$\beta$值，差别仅在截距上。 因此logit回归曲线在垂直方向上是平行的。
...
下边以四种奶酪的口味评分数据进行实例分析。一共有四种奶酪，评分分为1-9级。
```{r}
...
关于阈值，8个Trait分别对应的值即是，与plor的计算结果也是一致的。但是四种Cheese的回归系数，跟利用plor的计算结果正好是相反的。推测，ASReml直接把模型泛写为$logit(P(y_{i} \le j)) = ln(\frac{P(y_{i} \le j)}{P(y_{i} > j)})= \alpha_{j} + \beta_{1} x_{i1} +...+\beta_{k} x_{ik}$这种形式了。
