...
* 与之相反，AA基因型的频率为$q^{2}$，q=1-p，带有该基因型的个体的育种值uu=0a=0；
* TA基因型的频率为2pq，带有该基因型的个体的育种值u=1a。
* 该位点可解释的方差为$Var(u)=E(u^{2})-E(u)^{2}$。
* 在群体水平上，基于TT、TA和TT三种基因型的频率计算育种值u的平均值$E(u)=2ap^{2}+0q^{2}+a2pq\\=2ap^{2}+2apq\\=2ap^{2}+2ap(1-p)=2pa$，因此$E(u^{2})=(2pa)^{2}=4p^{2}a^{2}$。
* $E(u)^{2}$可以这样推导，上述三种基因型TT、TA、AA三种基因型育种值的平方和为$p^{2}(2a)^{2}+2pq(1a)^{2}+q^{2}(0a)^{2}=4p^{2}a^{2}+2pqa^{2}$：
* 再来推导方差$Var(u)=4p^{2}a^{2}-(4p^{2}a^{2}+2pqa^{2})=2pqa^{2}$。参考[Leggara的文章](http://nce.ads.uga.edu/wiki/lib/exe/fetch.php?media=gsip.pdf)Table 4。表中第三个基因型应为aa，PDF中错写为Aa。
* 两个体i，j的育种值可以表述为$m_{i}a，m_{j}a$，m表示MAF基因的含量。如果对其进行中心化，那么需要减掉群体均值$2pa$：$u_{i}=m_{i}a-2pa=(m_{i}-2p)a=z_{i}a; \\  u_{j}=m_{j}a-2pa=(m_{j}-2p)a=z_{j}a$。这样基本就推导出了Z为什么是M-P这个公式。
...
* 因此$u_{i}$和$u_{j}$之间的相关性表述为$r(u_{i},u_{j})=\frac{Cov(u_{i},u_{j})}{2p_{i}q_{j}\sigma^{2}}=\frac{z_{i}z_{j}\sigma^{2}}{2p_{i}q_{j}\sigma^{2}}=\frac{z_{i}z_{j}}{2p_{i}q_{j}}$。**实际上G矩阵表示的是个体间育种值的相关系数。**
