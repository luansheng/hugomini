...
使用的数据集为集成在R中datasets包中的mtcars。它是Motor Trend US 杂志收集的1973到1974年期间总共32辆汽车的11个指标: 油耗及10个与设计及性能方面的指标。

```{r}
print(mtcars)
```

选择hp变量（总功率）作为因变量，mpg（油耗）、wt（重量）、drat（后轴比）、qsec（提速能力1/4英里需要的时间）作为自变量。
...
通过K折交叉验证方法，计算test MSE。计算不同$\lambda$值对应的test MSE，选择最小test MSE对应的$\lambda$值。

cv.glmnet()函数可以执行多折交叉验证，如5折。
...
画出$\lambda$值对应的test MSE，对应图中x轴的log(best_lambda_s)=$log(best_lambda_s)$位置。
```{r SelectLambda, fig.cap="lambda值与test MSE的对应关系图"}
plot(cv_model)
```
...
成功建立了预测模型:
$$
hp = 472.44 - 3.29*mpg + 18.82*wt - 2.04*drat - 17.53*qsec
$$
...
```{r}
plot(ridge_reg_model, xvar = "lambda")
```
