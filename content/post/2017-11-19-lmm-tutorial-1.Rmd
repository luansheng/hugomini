...
本文中，为了便于理解，使用的数据集是来自csv文件[shrimp](https://luansheng.netlify.com/post/datasets/shrimp.csv)中的对虾育种数据（对原始数据已经进行了变换）。
...
```{r}
shrimp <- fread(input = "datasets/shrimp.csv",sep = ",",header = TRUE,stringsAsFactors = TRUE)
str(shrimp)
```
